...
#find a way to include instructtions
...
def bottom():
    playsound('bottom.mp3', False)

def top():
    playsound('top.mp3', False)
...
def point():
    playsound('point.mp3', False)

def moving_l():
    playsound('m_left.mp3', False)

def moving_r():
    playsound('m_right.mp3', False)

def left_p():
    playsound('left_p.wav', False)

def right_p():
    playsound('right_p.wav', False)

def settings():
    playsound('settings.wav', False)

def paddleup():
    playsound('paddleup.mp3', False)

def paddledown():
    playsound('paddledown.mp3', False)

def zero():
    playsound('zero.mp3', False)
...
def versus():
    playsound('versus.mp3', False)

def do():
    playsound('Do.mp3', False)

def re():
    playsound('Re.mp3', False)

def mi():
    playsound('Mi.mp3', False)

def fa():
    playsound('Fa.mp3', False)

def sol():
    playsound('Sol.mp3', False)

def la():
    playsound('La.mp3', False)

def si():
    playsound('Si.mp3', False)

def do2():
    playsound('Do2.mp3', False)

def re2():
    playsound('Re2.mp3', False)

def levelone():
    playsound('levelone.mp3', False)

def level_easy():
    playsound('level_easy.mp3', False)

def leveltwo():
    playsound('leveltwo.mp3', False)

def levelthree():
    playsound('levelthree.mp3', False)
...
if mode == 'player':
    ...
    #idea = start playing instructions at beginning of game
...
def on_receive_ball(address, *args):
    ...
    if counter == 5:
        counter = 0
        if args[1] >= 400 and args[1] < 450:
            do()
        if args[1] >= 350 and args[1] < 400:
            re()
        if args[1] >= 300 and args[1] < 350:
            mi()
        if args[1] >= 250 and args[1] < 300:
            fa()
        if args[1] >= 200 and args[1] < 250:
            sol()
        if args[1] >= 150 and args[1] < 200:
            la()
        if args[1] >= 100 and args[1] < 150:
            si()
        if args[1] >= 50 and args[1] < 100:
            do2()
        if args[1] >= 0 and args[1] < 50:
            re2()
...
def listen_to_speech():
    ...
    while not quit:
        ...
        try:
            ...
            if recog_results == "play" or recog_results == "start" or recog_results == "play start" or recog_results == "start start":
    ...
    while not quit:
        ...
        if pitch != 0:
            ...
            if pitch >= 375 and pitch < 600:
                difference = (pitch - 375) * 1.14
                location = 225 - difference
                if location < 25:
                    location = 25
            if pitch < 375 and pitch >= 150:
                difference = (375 - pitch) * 1.34
                location = 225 + difference
                if location > 425:
                    location = 425
            if pitch < 150:
            ...
            client.send_message('/p', location) 
